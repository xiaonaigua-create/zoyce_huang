...
function autoBind(instance: Record<string, unknown>) {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== 'constructor' && typeof instance[key] === 'function') {
      ;(instance as Record<string, unknown>)[key] = (instance[key] as Function).bind(instance)
    }
  })
}
...
async function resolveFont(font?: string, fontUrl?: string) {
  ...
  try {
    ...
    if (document.fonts && document.fonts.load) { try { await document.fonts.load(resolved) } catch { /* ignore */ } }
    return resolved
  } catch (error) {
...
class Media {
  ...
  update(scroll: { ease: number; current: number; target: number; last: number }, direction: string) {
...
class App {
  ...
  boundOnResize: () => void
  boundOnWheel: () => void
  boundOnTouchDown: () => void
  boundOnTouchMove: () => void
  boundOnTouchUp: () => void
  boundOnKeyDown: () => void
  isDown = false
  start = 0
  ...
  onKeyDown(e: KeyboardEvent) {
    switch (e.key) {
      case 'ArrowRight':
        e.preventDefault(); this.scroll.target += this.scrollSpeed * 5; this.onCheckDebounce(); break
      case 'ArrowLeft':
        e.preventDefault(); this.scroll.target -= this.scrollSpeed * 5; this.onCheckDebounce(); break
      case 'Home':
        e.preventDefault(); this.scroll.target = 0; this.onCheckDebounce(); break
      default:
        break
    }
  }
  ...
  onResize() {
    ...
    if (this.medias) this.medias.forEach((media) => media.onResize({ screen: this.screen, viewport }))
  }
